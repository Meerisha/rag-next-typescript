...
interface Message {
  ...
  content: string;
  createdAt?: Date;
  agent?: string; // Added to track which agent responded
}
    ...
    <div className="flex flex-col h-screen bg-background">
      <header className="border-b border-border p-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Experience AI chat with enhanced agent capabilities and RAG integration
        </p>
      </header>
      ...
      >
        {messages.length === 0 ? (
          <div className="text-center text-muted-foreground mt-8">
            ...
            <p className="text-lg mb-2">Welcome to OpenAI Agents SDK Chat!</p>
            <p>Start a conversation by typing a message below.</p>
            <p className="text-xs mt-2">
              This demo uses the AI SDK adapter with OpenAI Agents SDK
            </p>
            <div className="mt-4 p-3 bg-muted rounded-lg text-left max-w-md mx-auto">
              <p className="text-sm font-medium mb-2">✨ Features:</p>
              <ul className="text-xs space-y-1">
                <li>• Agent-powered responses with enhanced reasoning</li>
                <li>• RAG integration with document sources</li>
            ...
            <div key={message.id} className="space-y-2">
              ...
              >
                <div className="max-w-md lg:max-w-2xl">
                  {/* Agent info and sources at the top for assistant messages */}
      ...
      <form onSubmit={handleSubmit} className="border-t border-border p-4">
        ...
        <p className="text-xs text-muted-foreground mt-2 text-center">
          Powered by OpenAI Agents SDK with AI SDK adapter
        </p>
      </form>
    </div>
  );
